import { collectDataEyeRankingDetailed } from "./collectors/live.js";
...
| 榜单类型 | ${result.rankType || "all"} |
| 周期 | ${result.period || "all"} |
| 状态 | ${result.status} |
| 预检条数 | ${result.preview?.rows?.length || 0} |
| 预检组合数 | ${result.preview?.combos?.length || 0} |
...
${renderComboSummary(result.preview?.combos || [])}
...
async function previewDataEye({ rankingDate, rankType, period }) {
  try {
    ...
    return {
      ok: details.rows.length > 0,
      rows: details.rows,
      combos: details.combos,
      error: details.rows.length > 0 ? "" : "DataEye 预检未返回榜单行。",
      action: details.rows.length > 0 ? "" : "请重新抓包或检查接口响应结构。"
    };
  } catch (error) {
    return {
      ok: false,

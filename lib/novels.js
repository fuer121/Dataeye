...
export function listNovels(query = "", options = {}) {
  ...
  if (trimmed) {
    where.push(`
      (
        n.novel_name LIKE @query
        OR n.book_id LIKE @query
        OR n.author LIKE @query
        OR EXISTS (
          SELECT 1
          FROM novel_mappings nm_search
          WHERE nm_search.novel_name = n.novel_name
            AND nm_search.drama_title LIKE @query
        )
      )
    `);
    params.query = `%${trimmed}%`;
  ...
  if (match === "matched") {
    where.push(`
      EXISTS (
        SELECT 1
        FROM novel_mappings nm_match
        WHERE nm_match.novel_name = n.novel_name
      )
    `);
  ...
  const rows = db
    .prepare(`
      SELECT
        ...
        n.author,
        n.category_level1 AS categoryLevel1,
        n.category_level2 AS categoryLevel2,
        n.reader_preference AS readerPreference,
        n.source_ref AS sourceRef,
        n.created_at AS createdAt,
        n.updated_at AS updatedAt,
        n.normalized_novel_name AS normalizedNovelName,
        EXISTS (
          SELECT 1
          FROM novel_mappings nm_sort
          WHERE nm_sort.novel_name = n.novel_name
        ) AS mappingMatched
      FROM novels n
      WHERE ${whereSql}
      ORDER BY mappingMatched DESC, n.updated_at DESC, n.id DESC
    `)
    .all(params);
  ...
  for (const mapping of listMappingsForNovels(db, rows)) {
    const novelName = cleanTitle(mapping.novelName);
...
function listMappingsForNovels(db, rows) {
  if (!rows.length) return [];
  ...
  const placeholders = rows.map((row, index) => {
    const key = `novelName${index}`;
    params[key] = cleanTitle(row.novelName);
    return `@${key}`;
  });
  ...
  return db
    .prepare(`
      SELECT novel_name AS novelName, drama_title AS dramaTitle
      FROM novel_mappings
      WHERE novel_name IN (${placeholders.join(", ")})
      ORDER BY updated_at DESC, id DESC
    `)
    .all(params);
}

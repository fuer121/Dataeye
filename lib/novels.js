...
export function listNovels(query = "", options = {}) {
  const db = getDb();
  const trimmed = query.trim();
  const match = String(options.match || "all");
  ...
  return rows
    ...
    .filter((row) => {
      if (match === "matched" && !row.mappingMatched) return false;
      if (match === "unmatched" && row.mappingMatched) return false;
      if (!queryText) return true;

      return (
        String(row.novelName || "")
          .toLowerCase()
          .includes(queryText) ||
        row.dramaTitles.some((title) =>
          String(title || "")
            .toLowerCase()
            .includes(queryText)
        )
...
export function normalizeNovelMappingRows(rows) {
  ...
  return rows
    .map((row) => {
      ...
      const platformId = readPlatformId(row);
...
function ensureNovelsForMappings(mappings, sourceRef = "mapping-import") {
  const db = getDb();
  const findByNormalizedName = db.prepare("SELECT id, platform_id AS platformId FROM novels WHERE normalized_novel_name = ?");
  const insert = db.prepare(`
    INSERT INTO novels (
      platform_id, novel_name, normalized_novel_name, source_ref, updated_at
    )
    VALUES (
      @platformId, @novelName, @normalizedNovelName, @sourceRef, CURRENT_TIMESTAMP
    )
  `);
  const updateMissingPlatformId = db.prepare(`
    UPDATE novels
    SET
      platform_id = @platformId,
      updated_at = CURRENT_TIMESTAMP
    WHERE id = @id
      AND (platform_id IS NULL OR TRIM(platform_id) = '')
  `);
  ...
  const run = db.transaction((rows) => {
    ...
    for (const row of rows) {
      const novelName = cleanTitle(row.novelName);
      const platformId = cleanText(row.platformId);
      ...
      if (existing) {
        if (platformId && !cleanText(existing.platformId)) {
          updateMissingPlatformId.run({
            id: existing.id,
            platformId
          });
        }
...
function readPlatformId(row) {
  return cleanText(readField(row, ["platformId", "platform_id", "平台ID", "平台id", "平台 Id", "平台 id"]));
}

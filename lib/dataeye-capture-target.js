const DATAEYE_RANKING_PATHS = new Set([
  "/playlet/motionComic",
...
export function isDataEyeRankingRequest(request) {
  try {
    const url = new URL(request?.url || "");
    if (url.hostname !== "playlet-applet.dataeye.com") return false;
    if (!DATAEYE_RANKING_PATHS.has(url.pathname)) return false;
    if (url.pathname === "/playlet/motionComic" && !url.searchParams.has("rankType")) return false;
    return Boolean(getDataEyeRequestPeriodKey(request));
  } catch {
    return false;
  }
}
...
export function compareDataEyeMotionComicRequests(a, b) {
  return compareDataEyeRankingRequests(a, b);
}

export function compareDataEyeRankingRequests(a, b) {
  ...
  return getDataEyeRankingSpecificity(b) - getDataEyeRankingSpecificity(a);
}
...
function getDataEyeRankingSpecificity(request) {
  try {
    const url = new URL(request?.url || "");
    let score = 0;
    if (url.searchParams.has("day")) score += 3;
    if (url.searchParams.get("rankType") === "0") score += 2;
    if (DATAEYE_RANKING_PATHS.has(url.pathname)) score += 1;

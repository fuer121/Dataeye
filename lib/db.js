...
export function initSchema(db = getDb()) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS ranking_entries (
      ...
      source TEXT NOT NULL CHECK (source IN ('dataeye', 'hongguo', 'native')),
    ...
    CREATE TABLE IF NOT EXISTS novels (
      ...
      book_id TEXT,
      novel_name TEXT NOT NULL,
      normalized_novel_name TEXT NOT NULL,
      author TEXT NOT NULL DEFAULT '',
      category_level1 TEXT NOT NULL DEFAULT '',
      category_level2 TEXT NOT NULL DEFAULT '',
      reader_preference TEXT NOT NULL DEFAULT '',
      source_ref TEXT NOT NULL DEFAULT 'file-import',
      created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
      UNIQUE (book_id),
      UNIQUE (normalized_novel_name)
    );

    CREATE INDEX IF NOT EXISTS idx_novels_search
      ON novels (novel_name, book_id, author);

    CREATE TABLE IF NOT EXISTS watch_states (
      normalized_title TEXT PRIMARY KEY,
      title TEXT NOT NULL,
      status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'followed', 'ignored')),
...
function migrateRankingEntriesDimensions(db) {
  ...
  const hasNativeSource = rankingEntriesAllowsNativeSource(db);
  ...
  if (hasRequiredColumns && hasExpectedUnique && hasNativeSource) return;
...
function rankingEntriesAllowsNativeSource(db) {
  const row = db
    .prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'ranking_entries'")
    .get();
  return /'native'/.test(row?.sql || "");
}

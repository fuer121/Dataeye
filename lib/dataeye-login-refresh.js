...
import { getDataEyeRequestRankingDate, isDataEyeRankingRequest } from "./dataeye-capture-target.js";
...
function isDataEyeMotionComicLoginSource(request) {
  try {
    const headers = request.headers || {};
    return isDataEyeRankingRequest(request) && Boolean(getDataEyeRequestRankingDate(request)) && Boolean(headers.authentication);
  } catch {
    return false;
  }
}

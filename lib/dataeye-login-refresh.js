...
未找到 DataEye 小程序目标请求。请先将 Charles/Proxyman 导出的剧查查小程序「漫剧热播榜 + 日榜」HAR、JSON 或 cURL 放入 \`captures/\`。
...
function buildResultMessage({ rankingDate, previewReady, skipPreview }) {
  if (!rankingDate) return "目标请求缺少 day 日期参数。";
...
function renderNextStep({ result, skipPreview }) {
  if (!result.rankingDate) {
    return "- 重新导出带 `day` 日期参数的剧查查小程序目标请求。";
  }

  if (result.status === "stale_capture") {
    return `- 重新打开剧查查小程序，进入「榜单 > 漫剧热播榜 > 日榜」。
...
function isDataEyeMotionComicLoginSource(request) {
  try {
    ...
    return (
      ...
      Boolean(headers.authentication)
    );
  } catch {
    return false;
  }

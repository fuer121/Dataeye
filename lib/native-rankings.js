...
export function importNativeRankings({ exportDate, rankingDate, baseDir } = {}) {
  ...
  validateNativePeriodRows(periods);
...
function validateNativePeriodRows(periods) {
  const signatures = periods.map((item) => ({
    ...item,
    signature: JSON.stringify(item.rows.map((row) => [row.title, row.heatValue]))
  }));
  ...
  for (let leftIndex = 0; leftIndex < signatures.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < signatures.length; rightIndex += 1) {
      ...
      if (left.rowCount > 0 && left.signature === right.signature) {
        throw new Error(
          `原生短剧 Excel 周期内容疑似重复：${PERIOD_FILES[left.period]} 与 ${PERIOD_FILES[right.period]} 数据完全相同`
        );
      }
    }
  }

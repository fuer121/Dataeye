...
function mapDataEyeMotionComicRow(item, context) {
  ...
  const heatValue = String(item.playCount || item.playCountAdd || "").trim();
  ...
  if (!Number.isFinite(rank) || !title || !heatValue || !Object.prototype.hasOwnProperty.call(item, "tags")) {
    throw new CollectorUnavailableError(
      `DataEye / 剧查查榜单字段缺失：rank=${item.ranking ?? ""}, title=${item.playletName ?? ""}, heat=${item.playCount ?? item.playCountAdd ?? ""}, tags=${item.tags ?? ""}`
    );
  }

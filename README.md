...
- `/`：榜单数据页，默认展示本地已有的最新榜单日期。DataEye Tab 支持日期、匹配状态、数据性质、榜单类型和周期筛选；站内原生短剧 Tab 使用榜期筛选。数据性质包括模拟、抓包导入和真实 live。`刷新登录态并预检` 操作入口默认不展示；重新抓包流程可通过 `?showDataEyeLoginRefresh=1` 显式打开。该入口只刷新本地 `.env.local.dataeye` 并执行预检，不会自动落库。
...
导入前会校验三份 Excel 均包含 `短剧名称` 和 `消耗`，并拦截 day/week/month 任意两份规范化后完全相同的文件，避免 BI 下载误选图表后把同一份数据导入多个周期。
...
也兼容 `平台ID`、`平台 id`、`小说名`、`书名`、`短剧/漫剧名`、`短剧名称`、`漫剧名称`。映射 Excel 中不存在于小说主库的小说会自动创建最小小说行；如果映射 Excel 提供平台 id，会写入小说主表的 `platform_id`。已存在小说的 `platform_id` 为空时会补齐，已有非空平台 id 不会被映射文件覆盖。

...
这是一个本地后台网站，用于采集、存储、匹配和核对短剧/漫剧榜单数据。当前版本已经跑通 SQLite 存储、站内原生短剧 Excel 导入、剧查查真实榜单落库、小说库导入、小说映射和精确匹配展示。
...
- `/`：榜单核对页，顶层来源为 `站内原生短剧` 和 `剧查查`。站内原生短剧使用榜期筛选；剧查查支持日期、匹配状态、数据性质、榜单类型和周期筛选。表格展示榜期、排名、作品名、热度值或消耗、小说匹配结果、平台 id、数据性质和采集时间。
- `/novels`：小说库页面，支持上传本地 Excel/CSV/JSON 小说主库，也支持单独上传映射 Excel，展示小说主库表格，并在同一页面按小说维护对应短剧/漫剧映射。列表支持按小说名称或短剧/漫剧名称搜索，并按映射状态筛选。

最新 DataEye 抓包为 fresh 时，剧查查页面会显示 `刷新登录态并预检` 操作入口；它只刷新本地 `.env.local.dataeye` 并执行预检，不会自动落库。
...
- 小说库页的 `导入小说主库`（Excel/CSV/JSON，即原 `导入 Excel/CSV` 能力）
- 小说库页的 `导入映射 Excel`
...
先执行只读预检，不写入 SQLite。同一日期、同一来源预检成功后，再执行 live 落库：
...
页面不展示当前筛选预检、抓包导入或固定全量采集入口；需要预检、抓包导入或采集时使用 CLI/API。固定全量能力仍可通过 CLI/API 执行 `rankType=all + period=all`；`rankType=all` 当前只覆盖已重新抓包确认的 4 个每日目标榜单。`collect:preview` 和 `collect:live` 仍兼容旧用法，默认只采历史 `rankType=0 + day`；`dataeye:daily` 默认执行新的 `rankType=all + period=all`。红果只保留为历史/模拟数据筛选和后续抓包分析入口，不推进单独红果 live。
...
剧查查页面日期和榜期是两个字段：`rankingDate` 用于页面日期筛选，保持本次采集日期；`periodValue` 用于展示真实日榜/周榜/月榜榜期。页面不提供榜期文本筛选入口，但表格仍展示榜期，URL/API 仍保留 `periodValue` 查询能力。例如 `2026-06-11` 采集的热力榜日榜请求参数是 `day=2026-06-10`，入库时 `rankingDate=2026-06-11`、`periodValue=2026-06-10`。

本机已落库的剧查查 live 数据以当前 SQLite 为准；可通过页面最新榜期或 `npm run mvp:status` 查看。例如 `2026-06-06` 旧 HAR 登录态重新预检曾返回 `DATAEYE_AUTH_EXPIRED` / `登录态已失效，请重新登录`；出现同类结果时，需要重新打开剧查查小程序、用 Charles/Proxyman 导出新 HAR，并更新 `.env.local.dataeye` 后再采集新日期。

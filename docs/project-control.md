...
| 2026-06-16 | DataEye 登录态恢复模块默认隐藏 | 日常榜单核对页不应因目录里存在 fresh HAR 自动展示登录态恢复操作，避免干扰主流程 | 仅在重新抓包流程显式带 `showDataEyeLoginRefresh=1` 或 `refreshLogin=1` 时展示恢复模块；刷新 API 和 CLI 不变 |
...
| T-043 | DataEye 登录态恢复模块按重新抓包流程显示 | 完成 | 实现型 Agent + 验证型 Agent | 用户页面反馈、现有 DataEye 登录态刷新 API | `app/page.jsx` 新增显式显示开关，`components/DashboardClient.jsx` 默认隐藏恢复模块，README 和测试同步 | 常规 DataEye 榜单页不展示登录态恢复模块；带 `showDataEyeLoginRefresh=1` 且存在 fresh/过期状态时才展示；API/CLI 保持可用 |
